...
> Have you ever had only 5 minutes to hack into a high security intranet to stop nuclear missiles from hitting ground due to some cyber terrorist activity?

<img src="./img/hacker.gif" width="400"/>
...
## A dark souless void with a blinking cursor

<img src="./img/GUI_CLI.jpg" width="400" alt="GUI vs CLI"/>

+ `Rstudio` as a GUI is also called Integrated Development enviroment (IDE). It has a CLI integrated under the `Terminal` tab.
+ `R` is running on the `Console` tab in Rstudio. Which is a CLI or terminal where R as program has been executed.
...
> **Factors** are special class of vector that can only contain predefined values. They represent categorical data.
...
- `a` is a element name such a column name in a dataframe
...
- can be coerced to other type if possible using `as._(x)`
...
+ `next` : starts the next iteration immediatly from the beginning of the current loop.
...
> This are more efficient and fast ways to iterate in R. Always use if possible.
...
Different stlyes of building the structure and elements of computer programs. Inspired from Wikipedia:
