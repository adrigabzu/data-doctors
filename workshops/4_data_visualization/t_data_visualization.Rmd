...
Use the `ToothGrowth` dataset (remember you can use `as_tibble()` to see the dataframe):
```r
...
> Tip: As seen in the previous code you might have to turn some continous variables into factors.
...
When the values are comparable and all of the same type, heatmaps can give us a good view of our numeric data. R have a built in heatmap function (`heatmap`) that can cover basic heatmaps. Let's make a heatmap of based on the categorical variables. Check the `?heatmap` for help.
...
There are many others [heatmap libraries](https://www.biostars.org/p/205417/). A well documented library for more complex heatmaps is [ComplexHeatmap](https://jokergoo.github.io/ComplexHeatmap-reference/book/)
